...
const createOrder = (order) => async (dispatch, getState) => {
  try {
    ...
    const { data: { data: newOrder } } = await Axios.post(`${API_BASE_URL}/api/orders`, order, {
      headers: {
...
const listMyOrders = () => async (dispatch, getState) => {
  try {
    ...
    const { userSignin: { userInfo } } = getState();
    const { data } = await Axios.get(`${API_BASE_URL}/api/orders/mine`, {
      headers:
        { Authorization: 'Bearer ' + userInfo.token }
    });
...
const payOrder = (order, paymentResult) => async (dispatch, getState) => {
  try {
    ...
    const { userSignin: { userInfo } } = getState();
    const { data } = await Axios.put(`${API_BASE_URL}/api/orders/` + order._id + "/pay", paymentResult, {
      headers:
        { Authorization: 'Bearer ' + userInfo.token }
    });
...
const deleteOrder = (orderId) => async (dispatch, getState) => {
  try {
    ...
    const { userSignin: { userInfo } } = getState();
    const { data } = await Axios.delete(`${API_BASE_URL}/api/orders/` + orderId, {
      headers:
        { Authorization: 'Bearer ' + userInfo.token }
    });
